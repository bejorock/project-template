...
export default function Paging({ data, count, limit, onChange }: PagingOpts) {
  ...
  for (let i = 0; i < totalPages; i++) {
    pages.push(
      <a
        href="#"
        className={currentIndex == i + 1 ? "index active" : "index"}
        key={i}
        onClick={(e) => {
          e.preventDefault();
          onChange(i + 1);
  ...
  useEffect(() => {
    const startSlice_ = currentIndex - 2 >= 0 ? currentIndex - 2 : (currentIndex - 1 >= 0 ? currentIndex - 1 : 1)
    const endSlice_ = currentIndex + 2 <= totalPages ? currentIndex + 2 : (currentIndex + 1 <= totalPages ? currentIndex + 1 : totalPages)
      ...
      <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
        ...
        <div>
          <nav className="pagination" aria-label="Pagination">
            ...
            {pages.slice(startSlice - 1 > 0 ? startSlice - 1 : 0, endSlice).map((index) => index)}
